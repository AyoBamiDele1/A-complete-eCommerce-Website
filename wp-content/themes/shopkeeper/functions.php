...
add_action( 'after_setup_theme', 'shopkeeper_set_product_columns_and_rows' );
function shopkeeper_set_product_columns_and_rows() {

	if( !get_option('shopkeeper_product_columns_updated', false) ){

		$products_per_column = get_theme_mod('products_per_column');

		if( isset( $products_per_column ) && $products_per_column != "" ) {
			update_option( 'woocommerce_catalog_columns', $products_per_column );
			update_option('shopkeeper_product_columns_updated', true);
		}
	}

	if( !get_option('shopkeeper_product_rows_updated', false) ){

		$products_per_page = get_theme_mod('products_per_page');

		if( isset( $products_per_page ) && $products_per_page != "" ) {
			$rows_per_page = $products_per_page / get_option('woocommerce_catalog_columns', 5);
			update_option( 'woocommerce_catalog_rows', (int)$rows_per_page );
			update_option('shopkeeper_product_rows_updated', true);
		}
	}

	if( (float)get_option( 'woocommerce_version' ) >= 3.3 ) {
	...
	if( (float)get_option( 'woocommerce_version' ) < 3.3 ) {
		...
		$products_per_page = (int)get_option( 'woocommerce_catalog_rows', 5) * (int)get_option( 'woocommerce_catalog_columns', 5);
